...
export const generateInfra = (root, options, sls, packageJson) => {
  ...
  if (extensions.includes("webpack")) {
    ...
    root.file("webpack.config.js", `const slsw = require('serverless-webpack');
...
`);
  ...
  if (extensions.includes("dynamodb")) {
    ...
    src.folder("lib").file("database.js", `import AWS from 'aws-sdk';
...
`);
    ...
    infra.folder("database").file("table.yml", `Resources:
  Table:
...
`);
  ...
  if (extensions.includes("cognito")) {
    ...
    sls.resources.push(`$\{file(./src/infrastructure/users/user-pool-client.yml)}`);
    sls.resources.push(`$\{file(./src/infrastructure/users/user-pool-domain.yml)}`);
    sls.custom.callbackUrl = {
      "default": `https://${domain || "my-domain.com"}`,
      "local": "http://localhost:3000"
    };
    const cognito = infra.folder("users");
    cognito.file("user-pool.yml", `Resources:
  UserPool:
    ...
    Properties:
      ...
      Schema:
        ...
        #   Required: false
`);
    cognito.file("user-pool-client.yml", `Resources:
  UserPoolClient:
    ...
    Properties:
      ...
      LogoutURLs:
        - $\{self:custom.callbackUrl.$\{self:custom.stage}, self:custom.callbackUrl.default}
`);
    cognito.file("user-pool-domain.yml", `Resources:
  HostedUi:
...
`);
  ...
  src.folder("lib").file("output.js", `export const success = (data, statusCode = 200) => {
  return {
    statusCode,
    body: JSON.stringify({
      data
